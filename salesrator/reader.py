...
def cube_from_excel(filename,sheets,index_col=None):
    framedict={}
    for sheet in sheets:
        framedict[sheet]=pd.io.excel.read_excel(filename,sheet,index_col=index_col)
    result=cube(framedict)
    return result
...
# 		tinsts = [Orange.data.Instance(tdomain, [str(i)]) for i in series]
